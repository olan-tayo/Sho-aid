...
const Tab = ({ children, onSelectedTab, data, tabContainer, tabText }: TabType) => {
    ...
    <div>
      <div className='flex gap-3'>
        {data?.map((tab, index) => {
          return (
            <div key={index}>
              <div className={`${tabContainer}`} onClick={() => handleSelectTab(tab)}>
                {' '}
          ...
          return (
            <>
              {tab?.value === selectedTab?.value && (
                <div key={index}>
                  <div>{children[selectedTab.key ? selectedTab.key - 1 : 1]}</div>
                </div>
              )}
            </>
          )
